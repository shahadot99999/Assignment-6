...
const displayCategoriesId = (datas) =>{
    // input field
        ...
        for(const data of dataa){
            ...
            <div class="col-md-8">
              <div class="card-body">
                ...
                <p class="card-text">${data.details.length>300?data.details.slice(0,300) + '....' : data.details}</p>
              </div>
              <div class="d-flex">
               <div> 
...
const loadModal =async (news_id) =>{
   try{
    ...
    modalTitle.innerText = data.data[0].title.length>50? data.data[0].title.slice(0,45) + '...' :data.data[0].title;
